...
<div class="login-wrap">
    <div class="login">
        ...
        <hr />
        <form class="needs-validation no-cross" action="{{ url(__('customer/login.routes.PASSWORD_UPDATE')) }}" method="post" novalidate>
          @csrf
          <input type="hidden" name="token" value="{{ $token }}">
            ...
            <div class="form-row">
                ...
                <div class="col-md-12 mb1">
                    <label for="validationCustom02">{{ __('customer/login.NEW_PASSWORD') }}<span class="star">&#160;*</span></label>
                ...
                <div class="col-md-12 mb1">
                    <label for="validationCustom02">{{ __('customer/login.CONFIRM_PASSWORD') }}<span class="star">&#160;*</span></label>
                    <input type="password" class="form-control" name="password_confirmation" id="validationCustom02" placeholder="" value="" required>
                    <div class="invalid-feedback">
                        {{ __('customer/login.CONFIRM_PASSWORD_REQUIRED') }}
                    </div>
                </div>
            </div>
            ...
            <div class="form-group">
                <div class="text-center reset-btn" >
                    <button type="submit" class="btn btn-primary btn-block btn-outline-primary">{{ __('customer/login.RESET') }}</button>
                </div>
            </div>
        </form>
    ...
    <div class="other-links form-group">
        <ul>
          <li><a href="{{ url(app()->getLocale()) }}" >@lang('customer/login.BACK_TO_LOGIN')</a></li>
        </ul>
    </div>
</div>
@endsection
