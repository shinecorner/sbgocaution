...
export default{
    ...
    watch: {
        date (val) {
            ...
            this.$store.dispatch("addInputItem", {fieldname: 'created_from', fieldvalue: this.dateFormatted});
        },   
        is_reset_form(val){
            if(val === true){
                this.dateFormatted = "";
    ...
    methods:{
        changeDate: function(val){            
            this.$store.dispatch("deleteInputItem", {fieldname: 'created_from'});
            if((typeof val !== "undefined") && (val.length == 10)){
                if(moment(val,"DD.MM.YYYY").isValid()){
                    this.date = this.parseDate(val);
                    this.$store.dispatch("addInputItem", {fieldname: 'created_from', fieldvalue: val});
                }                
            }            
        },
        formatDate (date) {
