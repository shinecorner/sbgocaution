...
export default{
    ...
    watch: {
        date (val) {
            ...
            this.$store.dispatch("addInputItem", {fieldname: 'birthdate', fieldvalue: this.dateFormatted});
        },
        is_reset_form(val){
            if(val === true){                
                this.date = "";
    ...
    methods:{
        changeDate: function(val){                        
            this.$store.dispatch("deleteInputItem", {fieldname: 'birthdate'});
            if((typeof val !== "undefined") && (val.length == 10)){
                if(moment(val,"DD.MM.YYYY").isValid()){
                    this.date = this.parseDate(val);
                    this.$store.dispatch("addInputItem", {fieldname: 'birthdate', fieldvalue: val});
                }                
            }            
        },
        formatDate (date) {
