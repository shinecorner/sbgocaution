				...
				<vuetable ref="vuetable"
                                ...
                                <template slot="enabled" slot-scope="props">
                                    <v-tooltip top v-if="props.rowData.status == '1'">
                                            <a href="#" @click="changeStatus(props.rowData)" slot="activator">
                                                <v-avatar size="26" class="round-badge-success">
    ...
    components: {
     ...
     computed:{     
      ...
      methods: {
        ...
        changeStatus(rowDataObj){
            let that = this;            
            this.loading = true;
            api.put('/api/users/change_status/'+rowDataObj.id) .then(function (response) {
                if((typeof response.data.data !== "undefined") && (response.data.data.hasOwnProperty('status'))){
                    rowDataObj.status = response.data.data.status;                                      
                    that.loading = false;
